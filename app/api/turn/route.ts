...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (locationChanged && process.env.ENABLE_IMAGE_GENERATION === 'true') { try { sceneImage = await generateScene(apiKey, `Strict 2D pixel art background for a fantasy RPG viewed in FIRST PERSON, handheld 32-bit Game Boy Advance-era visual language. Scene: ${next.location}. The ${state.className} adventurer is implied through the viewpoint; do not show a character portrait. Large crisp square pixels, visible pixel clusters, hard pixel edges, limited 16-bit handheld color palette, sprite-art textures, tile-map composition, dithering, strong readable silhouettes, cinematic but compact game background. Absolutely no photorealism, no realistic painting, no 3D render, no smooth gradients, no blurry airbrush, no text, no UI, no logo, no watermark.`); } catch (error) { console.error('INFINITA image generation failed', error); } }
