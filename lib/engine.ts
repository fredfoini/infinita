...
export function initialState(characterName = 'Viajante', campaignName = 'Nova campanha', className: ClassName = 'Explorador'): GameState {
  const classSkill: Record<string, string> = { Guerreiro: 'Combate', Explorador: 'Sobrevivência', Ladino: 'Furtividade', Místico: 'Arcana' };
  ...
  const specialty = classSkill[className] || 'Sobrevivência';
  skills[specialty] = { xp: 20, level: 1, trained: true };
  ...
  const opening = start[className] || `Você chega a Valedouro como ${className}, carregando uma história que ninguém ali conhece.`;
  return { campaignId: id(), campaignName, characterName, className, hp: 12, maxHp: 12, level: 1, xp: 0, xpToNext: cap(1), gold: 8, location: 'Vila de Valedouro', day: 1, hour: 18, inventory: ['3 rações', 'Tocha'], skills, titles: [], reputation: { global: 0, local: 0, regional: 0, npcs: {} }, events: [], log: [opening] };
}
